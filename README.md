...
1. Common Module(공통 모듈)
   - [x] 공통 Entity 구현 : 생성일, 수정일에 따른 공통 entity 분리하여 구현
   - [x] Enum 분리 : ErrorCode, RoleType, TokenType, PaymentStatus의 Enum을 구현
   - [x] Exception 구현 : 예외 처리를 하기 위해서 BusinessException, ErrorResponse, GlobalExceptionHandler를 구현

2. Member Module (회원 및 인증)
